...
class NewlistController extends Controller
{
    ...
    {   
          ...
          $series_list = Performers::orderBy('id','DESC')->paginate(12);

               
        return view('pages.newseries',compact('series_list'));
         
    }

    public function series_person()
    {   


           
       $series_list = Performers::Where('id',$_GET['id'])->orderBy('id','DESC')->paginate(12);
       $performer_lists=DB::table('movie_videos')->get();
      // dd($performer_lists);
       $recordarray=array();
       foreach ($performer_lists as $key => $value) {
         $arr=explode(",",$value->performer_id);
         if (in_array($_GET['id'], $arr)) {
           array_push($recordarray, $value);
         }
       }
         print_r($recordarray[0]);die;
       return view('pages.series_person',compact('series_list','series_lists'));
