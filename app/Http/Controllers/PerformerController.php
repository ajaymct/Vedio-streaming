...
class PerformerController extends Controller
{
    ...
    {   
            ...
            $movies_list = Performers::orderBy('id','DESC')->paginate(12);   
       // }   
            
       return view('pages.performer',compact('movies_list'));
         
    }
    public function become_performer()
    {   
          //  $movies_list = Performers::orderBy('id','DESC')->paginate(12);   
       // }   
            
       return view('pages.become_performer');
    ...
    public function add_performer(Request $request)
    {
       ...
       // dd('hii');die;
        ...
        if(!empty($inputs['id'])){
            ...
            $cat_obj = Performer::findOrFail($inputs['id']);

        }else{
            DB::enableQueryLog();
            $cat_obj = new Performer;

        }
       dd($inputs);
         $cat_obj->name = addslashes($inputs['fname']); 
         $cat_obj->email = $inputs['email']; 
         $cat_obj->gender = $inputs['gender']; 
         $cat_obj->age = $inputs['age']; 
         $cat_obj->location = $inputs['location']; 
         $cat_obj->height = $inputs['height']; 
         $cat_obj->weight = $inputs['weight']; 
         $cat_obj->experienceacting = $inputs['experienceacting'];
         $cat_obj->consideration = $inputs['consideration'];
         $cat_obj->number = $inputs['number'];
